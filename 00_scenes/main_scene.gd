	...
	if !DeliverySlot.SlotEmpty:
		if dialogueNumber==1:
			...
			if DeliverySlot.ItemReference.ItemID == 28:
				...
				dialogueNumber=dialogueNumber+1
				await get_tree().create_timer(5).timeout
				siguienteDialogo(character.CA,"El señor sospechoso dice:","Salve, oh, buen señor. Las buenas gentes hablan de usted como si fuera un santo. Obra milagros a nobles y mendigos sin distinción. 
Me encontraba en búsqueda de un material... Un polvo azabache proveniente de oriente... Se dice que provoca explosiones por doquier. ¿Tiene algo que pueda servirme?")
			else:
			...
			if DeliverySlot.ItemReference.ItemID == 18:
				...
				cajaDialogos.cambiarDialogo("Oh, jejeje... Perfecto buen señor... Dios le salve por su ayuda. Tenga cuidado con el uso de sus magias oscuras, podría meterle en... Problemas.
Dios sabrá si volvemos a encontrarnos.")
				cajaDialogos.animate_label()
				dialogueNumber=dialogueNumber+1
				await get_tree().create_timer(5).timeout
			...
			if DeliverySlot.ItemReference.ItemID == 13:
				...
				dialogueNumber=dialogueNumber+1
				await get_tree().create_timer(5).timeout
				siguienteDialogo(character.EA,"La artista frustrada dice:","Oh, ¿Qué es esto? ¿Es a caso un milagro? ¡O tal vez magia, brujería!
Buen señor, por favor, no quiero seguir viviendo en la entrada de la iglesia. Necesito algo que me permita dibujar y expresarme. ¡Así podré vender mis dibujos y dejar de restaurar arte sacro!")
			else:
			...
			if DeliverySlot.ItemReference.ItemID == 29:
				...
				dialogueNumber=dialogueNumber+1
				await get_tree().create_timer(5).timeout
				siguienteDialogo(character.FA,"El señor de otro juego dice:","Holiii~~ Jijijijiji.... Estamos- digo... estoy buscando algo para... protegerme de los desvergonzados que no me dejan tocar sus cosas... Jijiji, lalalala~~
Escuché que vuesa mercerd podía hacer magia... No espero menos de vos.")
			else:
			...
			if DeliverySlot.ItemReference.ItemID == 31 or DeliverySlot.ItemReference.ItemID == 06:
				...
				dialogueNumber=dialogueNumber+1
				await get_tree().create_timer(5).timeout
				siguienteDialogo(character.GA,"El señor con ojos raros dice:","¿Eh, hola? ¿Se encuentra alguien ahí? ¡Oh, buen señor, alabado sea!
Mire, realmente preciso de su ayuda. Un enano muy alto me retó a mirar al sol, ¡ese canalla sabía que mi vista empeoraría y  robó todas mis manzas!
Por favor, necesito algo que me ayude a ver mejor ¿Un hechizo pueda ser?")
			else:
